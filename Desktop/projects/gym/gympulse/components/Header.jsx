...
const Header = () => {
  ...
  if (isLoading || !isLoaded) {
    return (
      <div className='w-full h-16 border-b border-[#D4AF37]/10 flex items-center justify-between px-6 md:px-12'
  ...
  return (
    <div
      className='w-full flex items-center justify-between px-6 md:px-12 -mt-16 py-3 border-b border-[#D4AF37]/10 backdrop-blur-md sticky top-0 z-50'
      style={{ background: 'radial-gradient(ellipse 80% 60% at 30% 0%, #1A150F 0%, #111114 55%, #0D0D0F 100%)' }}
    >
      ...
      <nav className='hidden md:flex items-center gap-6 relative z-10'>
        {primaryLinks.map((value) => (
          <div key={value.id} className="relative group cursor-pointer">
            <Link href={value.link} className="font-sans text-sm tracking-wide text-gray-400 font-light transition-colors duration-300 group-hover:text-[#FFF5D6]">
              {value.title}
            </Link>
            <span className="absolute left-0 -bottom-1 w-0 h-px bg-gradient-to-r from-[#D4AF37] to-[#AA7C11] transition-all duration-300 group-hover:w-full" />
          </div>
        ))}
        ...
        <div className="relative">
          ...
          {isMoreOpen && (
            <div className="absolute top-8 left-0 w-52 bg-[#111114]/95 border border-[#D4AF37]/10 backdrop-blur-xl rounded-xl p-1.5 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
              {secondaryLinks.map(link => (
                <Link
                  key={link.id}
                  href={link.link}
                  onClick={() => setIsMoreOpen(false)}
                  className="block px-3 py-2 rounded-lg text-sm text-gray-400 hover:text-[#FFF5D6] hover:bg-white/[0.04] transition-colors"
                >
                  {link.title}
                </Link>
              ))}
          ...
          <div className='flex items-center gap-4'>
            ...
            <div className="relative">
              <button
                ...
                title="შეტყობინებები"
                className="p-2 rounded-xl bg-white/[0.03] border border-white/5 text-gray-500 hover:text-[#D4AF37] hover:border-[#D4AF37]/20 transition-all relative"
              >
                <Bell className="w-4 h-4" />
                {hasUnread && (
                  <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[#D4AF37] rounded-full animate-pulse" />
                )}
              </button>

              {isNotifOpen && (
                <div className="absolute right-0 top-11 w-80 bg-[#111114]/95 border border-[#D4AF37]/15 backdrop-blur-xl rounded-2xl p-4 shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-150">
                  <h3 className="text-[#D4AF37] text-[10px] uppercase tracking-widest mb-3 border-b border-white/5 pb-2">შეტყობინებები</h3>
                  <div className="max-h-56 overflow-y-auto space-y-1.5">
                    {notifications?.length === 0 ? (
                      ...
                      notifications?.map((n) => (
                        <div
                          key={n._id}
                          className={`p-2.5 rounded-lg cursor-pointer transition-colors ${n.isRead ? 'bg-white/[0.02]' : 'bg-[#D4AF37]/8'}`}
            ...
            <div className="p-[1px] rounded-full bg-gradient-to-b from-[#D4AF37]/30 to-transparent ml-1">
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-[#111114]/98 border-b border-[#D4AF37]/10 p-5 md:hidden flex flex-col gap-1 animate-in slide-in-from-top-3 duration-200">
          {allLinks.map((value) => (
            <Link
              key={value.id}
              href={value.link}
              className="text-base text-gray-400 py-2.5 px-3 rounded-lg hover:text-[#FFF5D6] hover:bg-white/[0.03] transition-colors border-b border-white/[0.03] last:border-0"
