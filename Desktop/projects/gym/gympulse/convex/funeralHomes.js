...
async function resolveFuneralHomeUrls(ctx, funeralHome) {
  ...
  const services = await Promise.all(
    (funeralHome.services || []).map(async (s) => ({
      ...s,
      imageUrls: s.images?.length
        ? await Promise.all(s.images.map((id) => ctx.storage.getUrl(id)))
        : [],
    }))
  );

  return { ...funeralHome, logoUrl, coverUrl, galleryUrls, services };
}
...
export const getMyFuneralHome = query({
  ...
  handler: async (ctx) => {
    ...
    return resolveFuneralHomeUrls(ctx, funeralHome); // CHANGED
  },
});
